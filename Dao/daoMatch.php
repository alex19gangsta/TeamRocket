...
class DaoMatch
{
    ...
    public function addMatch($match): int
    {
        ...
        $pdo = Dao::getConnection();     // Connection to the DB
        $request = $pdo->prepare("INSERT INTO matchs (heure,date,saison,Id_stade) VALUES ('" . $match->get_hours() . "','" . $match->get_date() . "','" . $match->get_season() . "','" . $match->get_idStadium() . "')");   // Prepare the request to be execute
    ...
    public function modifyMatch($match): void
    {
        ...
        $pdo = Dao::getConnection();     // Connection to the DB
        $request = $pdo->prepare("UPDATE matchs SET heure='" . $match->get_hours() . "',date='" . $match->get_date() . "',saison='" . $match->get_season() . "',Id_stade='" . $match->get_idStadium() . "' WHERE Id_match =" . $match->get_idMatch());    // Prepare the request to be execute
    ...
    {
        $pdo = Dao::getConnection();     // Connection to the DB
        $request = $pdo->prepare("DELETE FROM matchs WHERE Id_match=" . $idMatch); // Prepare the request to be execute
