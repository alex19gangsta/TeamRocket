...
class DaoTop
{
    public function showTopGoal($season): array
    {
        ...
        $tab = [];
        $request = $pdo->prepare("SELECT joueurs.Nom,joueurs.Prenom,SUM(jouer.But_marque_par_match)AS \"buts_totaux\"FROM joueurs INNER JOIN jouer ON joueurs.Id_joueur=jouer.Id_joueur 
        INNER JOIN matchs ON jouer.Id_match=matchs.Id_match WHERE matchs.Saison=\"" . $season . "\" GROUP BY joueurs.Id_joueur ORDER By buts_totaux DESC"); // Prepare the request to be execute
    ...
    public function showTopPresence($season): array
    {
        ...
        $request = $pdo->prepare("SELECT joueurs.Prenom, joueurs.Nom,COUNT(jouer.Id_match) AS \"presence\" FROM jouer INNER JOIN joueurs ON jouer.Id_joueur=joueurs.Id_joueur
         INNER JOIN matchs ON jouer.Id_match=matchs.Id_match WHERE matchs.Saison=\"" . $season . "\" GROUP BY jouer.Id_joueur ORDER BY presence DESC "); // Prepare the request to be execute
