...
class DaoStat
{
    public function showGoal($season): array
    {
        ...
        //                        SELECT SUM(But_marque_par_match) AS "total buts" FROM jouer INNER JOIN matchs ON jouer.Id_match=matchs.Id_match WHERE matchs.Saison= "2020/2021" AND jouer.Id_joueur=1 
    ...
    public function showPresence($season): array
    {
        $pdo = Dao::getConnection();     // Connection to the DB
        $tab = [];
        $request = $pdo->prepare("SELECT joueurs.Prenom, joueurs.Nom,COUNT(jouer.Id_match) AS \"presence\" FROM jouer INNER JOIN joueurs ON jouer.Id_joueur=joueurs.Id_joueur INNER JOIN matchs ON jouer.Id_match=matchs.Id_match WHERE matchs.Saison='" . $season . "' AND joueurs.Id_joueur=" . $_SESSION["PlayerID"] . " GROUP BY jouer.Id_joueur ORDER BY presence DESC ");
        // $request = $pdo->prepare("SELECT COUNT(jouer.Id_match) AS \"presence\" FROM jouer INNER JOIN joueurs ON jouer.Id_joueur=joueurs.Id_joueur INNER JOIN matchs ON jouer.Id_match=matchs.Id_match WHERE matchs.Saison= '2020/2021' AND joueurs.Id_joueur=1 GROUP BY jouer.Id_joueur ORDER BY presence DESC ");
        try {
            $request->execute();    // Execute the request
